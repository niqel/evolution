...
| **Execution** | Functional Concept | Evo Runtime coordination domain | RUNTIME DOMAIN CANDIDATE | Active work coordinated from Entry Point to finalization. |
...
| **Value** | Data Concept | EvoV / `evo-values` | STRONG AFFINITY | Valid data produced or transported across operations on common EvoV base; `Value != Failure`. |
| **Failure** | Outcome Concept | Evo Runtime coordination domain | RUNTIME DOMAIN CANDIDATE | Outcome indicating uncompleted or failed work; `Failure != Value`. |
| **Result** | Outcome Concept | Evo Runtime coordination domain | RUNTIME DOMAIN CANDIDATE | Outcome of an operation or execution (`Result != Value`, `Result != Failure`). |
...
> *Which of the 11 Runtime Domain Candidates actually require a concrete technical type in `evo-runtime`?*
