...
Para operaciones estructuradas sobre filas ya materializadas, la frontera pública conceptual incluye:

- `StructuredItems<'a>`: colección prestada de `FilesystemIterationItem` ya materializados.
- `Filter(StructuredItems<'a>, &FilterExpression) -> StructuredItems<'a>`: conserva únicamente los elementos que cumplen un predicado estructurado.
