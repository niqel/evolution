...
Debe existir un scope activo antes de ejecutar iter.
iter utiliza el scope activo como ámbito de trabajo.
En un filesystem scope, iter obtiene únicamente los elementos directamente contenidos en la ruta activa.
Los directorios encontrados se devuelven como elementos, pero su contenido no se recorre.
Cada elemento devuelto contiene información estructurada suficiente para identificar:
nombre
ruta
tipo de recurso
El resultado estructurado de iter es independiente de cómo Evo Shell lo muestre visualmente.
Si el scope no puede iterarse, Evo Shell debe informar que la operación no pudo resolverse.
La ejecución de iter no debe modificar el scope activo.
Ejemplo
