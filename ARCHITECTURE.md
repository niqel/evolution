...
- **Datos Prestados Internos Estables sin Orquestación**: Cuando una capacidad devuelve información interna estable como datos prestados (ej. `Inform` Use Case $\rightarrow$ `shell_informant` Collaborator $\rightarrow$ `ShellInformation<'static>`), la arquitectura fluye directamente `Use Case → Collaborator → Struct` sin requerir Agent (al no existir orquestación), Resolver, Contract ni Provider.
