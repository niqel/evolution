...
- **Datos Prestados Internos Estables sin Orquestación**: Cuando una capacidad devuelve información interna estable como datos prestados (ej. `Inform` Use Case $\rightarrow$ `shell_informant` Collaborator $\rightarrow$ `ShellInformation<'static>`, y `Welcome` Use Case $\rightarrow$ `shell_welcomer` Collaborator $\rightarrow$ `WelcomeInformation<'static>`), la arquitectura fluye directamente `Use Case → Collaborator → Struct` sin requerir Agent (al no existir orquestación), Resolver, Contract ni Provider. Los Collaborators pueden componerse localmente entre sí (ej. `shell_welcomer` invoca a `shell_informant::collaborate()`).
